			...
			struct mq_attr *attr)
{
	...
	if (ret)
		goto out;
	ret = vfs_create2(path->mnt, dir->d_inode, dentry, mode, NULL);
				...
				struct dentry *dentry, int oflag)
{
	...
	if (inode_permission2(path->mnt, dentry->d_inode, oflag2acc[oflag & O_ACCMODE])) {
		ret = -EACCES;
		goto err;
	}
		...
		struct mq_attr __user *, u_attr)
{
	...
	dentry = lookup_one_len2(name, mnt, ipc_ns->mq_mnt->mnt_root, strlen(name));
	if (IS_ERR(dentry)) {
